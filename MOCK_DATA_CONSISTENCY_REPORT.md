...
| `scripts/seed-personal.ts` | **Personal Seed** | **Partial** | Populates `User` with `masteryScores`, `topics`. **Missing** `badges`, `streak`. |
...
*   **Seed Scripts:** Neither `seed-database.ts` nor `seed-personal.ts` include these fields when creating `students` documents in Firestore.

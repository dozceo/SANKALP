...
import { docsTree, generateGraphData, flattenDocs, studentsData } from '@/data/studentsDataStatic';
...
export function InteractiveGraph({ onNodeClick, highlightedNode }: InteractiveGraphProps) {
  ...
  const fullGraphData = useMemo(() => generateGraphData(studentsData), []);
  ...
  const nodeColor = useCallback((node: ExtendedNodeObject) => {
    ...
    if (connectedNodes.has(node.id as string)) {
      return '#8b5cf6'; // Connected nodes - primary purple
    }
    return '#6d28d9'; // Other nodes - darker purple
  }, [highlightedNode, hoveredNode, connectedNodes]);

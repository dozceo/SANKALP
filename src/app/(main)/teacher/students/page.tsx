...
export default function StudentsPage() {
    ...
    const stats = useMemo(() => {
        ...
        for (const s of students) {
            if (s.progress < 60) atRiskCount++;
            if (isActive(s.lastActivity)) activeCount++;
        ...
        return {
            ...
            atRisk: atRiskCount,
            atRiskPercent: students.length > 0 ? Math.round((atRiskCount / students.length) * 100) : 0,
            avgPerformance,
            activeThisWeek: activeCount,
            activeRate: students.length > 0 ? Math.round((activeCount / students.length) * 100) : 0,
        };
    }, [students]);

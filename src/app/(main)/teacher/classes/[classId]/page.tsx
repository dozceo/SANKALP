...
export default function ClassDetailsPage() {
    ...
    const studentStats = useMemo(() => {
        // Optimization: Use Date.now() and Date.parse instead of instantiating new Date objects
        const now = Date.now();
        const activeStudents = students.filter(s => {
            if (!s.lastLoginDate) return false;
            const daysSinceLogin = (now - Date.parse(s.lastLoginDate)) / (1000 * 60 * 60 * 24);
            return daysSinceLogin <= 7;
        });

        const recentJoins = students.filter(s => {
            if (!s.joinedClassAt) return false;
            const daysSinceJoin = (now - Date.parse(s.joinedClassAt)) / (1000 * 60 * 60 * 24);
                            ...
                            <TableBody>
                                {filteredStudents.map((student) => {
                                    const daysSinceLogin = student.lastLoginDate
                                        ? (Date.now() - Date.parse(student.lastLoginDate)) / (1000 * 60 * 60 * 24)
                                        : null;

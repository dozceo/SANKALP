...
interface TopicCardProps {
    topic: string;
    signal: StudentIntelligence["mastery"][string];
}

const TopicCard = memo(function TopicCard({ topic, signal }: TopicCardProps) {
        ...
        <Card className="md:col-span-2 lg:col-span-3">
            ...
            <CardContent className="grid gap-4 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4">
                {masteryEntries.map(([topic, signal]) => (
                    <TopicCard key={topic} topic={topic} signal={signal} />
                ))}
            </CardContent>
        </Card>
    );
